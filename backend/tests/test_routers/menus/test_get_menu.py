...
def test_get_menu_success(client, db):
    ...
    # WHEN
    params = {"rest_id": 1}
    response = client.get(f"/api/v1/menus", params=params)
    ...
    menu_list_query = db.query(Menu).filter(Menu.rest_id == 1).all()
    assert len(menu_list_query) == 3
