...
def test_create_menu_success(client, db):
    ...
    response = client.post(f"/api/v1/menus", params=params, json=request_data)
    ...
    assert response.json()["name"] == "ASAP PIZZA S/L"


# ============ 실패 코드 ============
# 필드 누락
@pytest.mark.skip(reason="검증 완료")
def test_create_menu_without_field(client, db):
