...
def test_update_menu_success(client, db):
    ...
    # WHEN
    params = {"rest_id": 1}
    request_body = { "price": 25900 }
    response = client.patch(f"/api/v1/menus/1", params=params, json=request_body)
    ...
    menu = db.query(Menu)\
                        .filter(Menu.rest_id == 1,
                                Menu.id == 1).first()
    assert menu.price == 25900
