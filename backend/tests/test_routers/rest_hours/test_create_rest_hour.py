import pytest

from app.models.rests import Rest
from app.models.rest_hours import RestHour


# ============ 성공 코드 ============
# 식당 영업시간 등록 성공 코드
@pytest.mark.skip(reason="검증 완료")
def test_create_rest_hour_success(client, db):

    # GIVEN
    rest_list = [Rest(name="에이셉피자 홍대점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 서교동 403-14", "x": 0.215, "y": 4.1598}),
                Rest(name="에이셉피자 성수점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 성동구 성수동1가 22-5", "x": 0.215, "y": 4.1598}),
                Rest(name="피자스쿨 상수역점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 상수동 330-1", "x": 0.215, "y": 4.1598}),
                Rest(name="데코아발림본점", 
                      category="Cafe", 
                      addr={"addr_name": "서울 마포구 상수동 311-4", "x": 0.215, "y": 4.1598})]
    db.add_all(rest_list)
    db.commit()

    params = {"rest_id": 1}
    request_data = [
        {
            "weekday": "MON",
            "is_closed": False,
            "open_time": "10:00",
            "close_time": "20:00"
        },
        {
            "weekday": "SUN",
            "is_closed": True
        }
    ]


    # WHEN
    response = client.post(f"/api/v1/hours/bulk", params=params, json=request_data)


    # THEN
    print("\n================ [응답 결과] ================")
    print("Status Code:", response.status_code)
    print("Response JSON:", response.json())
    print("=============================================\n")

    assert response.status_code == 201
    assert response.json()[0]["weekday"] == "MON"

# ============ 실패 코드 ============
# 필수 값 누락
@pytest.mark.skip(reason="검증 완료")
def test_create_rest_hour_without_field(client, db):

    # GIVEN
    rest_list = [Rest(name="에이셉피자 홍대점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 서교동 403-14", "x": 0.215, "y": 4.1598}),
                Rest(name="에이셉피자 성수점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 성동구 성수동1가 22-5", "x": 0.215, "y": 4.1598}),
                Rest(name="피자스쿨 상수역점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 상수동 330-1", "x": 0.215, "y": 4.1598}),
                Rest(name="데코아발림본점", 
                      category="Cafe", 
                      addr={"addr_name": "서울 마포구 상수동 311-4", "x": 0.215, "y": 4.1598})]
    db.add_all(rest_list)
    db.commit()

    params = {"rest_id": 1}
    request_data = [
        {
            "weekday": "MON",
            "open_time": "10:00",
            "close_time": "20:00"
        },
        {
            "weekday": "SUN",
            "is_closed": True
        }
    ]


    # WHEN
    response = client.post(f"/api/v1/hours/bulk", params=params, json=request_data)


    # THEN
    print("\n================ [응답 결과] ================")
    print("Status Code:", response.status_code)
    print("Response JSON:", response.json())
    print("=============================================\n")

    assert response.status_code == 422

# 중복 요일
@pytest.mark.skip(reason="검증 완료")
def test_create_rest_hour_existing_weekday(client, db):

    # GIVEN
    rest_list = [Rest(name="에이셉피자 홍대점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 서교동 403-14", "x": 0.215, "y": 4.1598}),
                Rest(name="에이셉피자 성수점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 성동구 성수동1가 22-5", "x": 0.215, "y": 4.1598}),
                Rest(name="피자스쿨 상수역점", 
                      category="Restaurant", 
                      addr={"addr_name": "서울 마포구 상수동 330-1", "x": 0.215, "y": 4.1598}),
                Rest(name="데코아발림본점", 
                      category="Cafe", 
                      addr={"addr_name": "서울 마포구 상수동 311-4", "x": 0.215, "y": 4.1598})]
    db.add_all(rest_list)
    db.commit()

    rest_hour = RestHour(rest_id=1, weekday= "MON",
                        is_closed= False,
                        open_time= "10:00",
                        close_time= "20:00")
    
    db.add(rest_hour)
    db.commit()

    params = {"rest_id": 1}
    request_data = [
        {
            "weekday": "MON",
            "is_closed": False,
            "open_time": "10:00",
            "close_time": "20:00"
        }
    ]


    # WHEN
    response = client.post(f"/api/v1/hours/bulk", params=params, json=request_data)


    # THEN
    print("\n================ [응답 결과] ================")
    print("Status Code:", response.status_code)
    print("Response JSON:", response.json())
    print("=============================================\n")

    assert response.status_code == 409