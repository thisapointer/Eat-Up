...
def test_delete_rest_hour_success(client, db):
    ...
    # WHEN
    response = client.delete(f"/api/v1/hours/SUN", params=params)
    ...
    rest_hour_list_query = db.query(RestHour).filter(RestHour.rest_id == 1).all()
    assert len(rest_hour_list_query) == 6
