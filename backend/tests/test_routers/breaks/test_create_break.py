...
def test_create_break_success(client, db):
    ...
    # WHEN
    response = client.post(f"/api/v1/breaks/bulk", params=params, json=request_data)
    ...
    break_query = db.query(Break)\
                    ...
                    .filter(RestHour.rest_id == 1,
                            Break.weekday == "MON",
                            Break.id == 1).all()
    assert break_query[0].break_start_time == time(15, 0)

# ============ 실패 코드 ============
# 식당 브레이크 종료 시간 없이 등록
@pytest.mark.skip(reason="검증 완료")
def test_create_break_without_field(client, db):
...
# 중복 브레이크타임 등록
@pytest.mark.skip(reason="검증 완료")
def test_create_break_existing_weekday(client, db):
    ...
    break_list = [Break(weekday=rest_hour_list[0].weekday, 
